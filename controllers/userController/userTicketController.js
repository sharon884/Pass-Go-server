...
const  getAllSeatsByEvent = async ( req, res ) => {
    try {
        ...
        const seats = await Seat.find({
            event : eventId
        }).select("_id seatNumber status category lockExpiresAt ").lean();
    ...
    const result =     await Seat.updateMany(
        ...
        if ( result.modifiedCount !== seatIds.length ) {
            return res.status(STATUS_CODE.CONFLICT).json({
                success : false,
                message : "Some seats were just locked by another user.Please try again.",
            })
        }
       ...
       if ( io ) {
        ...
        const seatDetails = await Seat.find({ _id : {$in : seatIds} }).select("seatNumber").lean();
